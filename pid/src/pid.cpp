...
PidObject::PidObject() : error_(3, 0), filtered_error_(3, 0), error_deriv_(3, 0), filtered_error_deriv_(3, 0)
{
  ...
  node_priv.param<std::string>("topic_from_controller", topic_from_controller_, "cmd_vel");
  node_priv.param<std::string>("topic_from_plant", topic_from_plant_, "ground_truth/state");
  ...
  while( ros::ok() && !ros::topic::waitForMessage<geometry_msgs::PoseStamped>(setpoint_topic_, ros::Duration(10.)))
     ROS_WARN_STREAM("Waiting for first setpoint message.");

  while( ros::ok() && !ros::topic::waitForMessage<nav_msgs::Odometry>(topic_from_plant_, ros::Duration(10.)))
...
void PidObject::setpointCallback(const geometry_msgs::PoseStamped& setpoint_msg)
{
  setpoint_ = setpoint_msg;

  new_state_or_setpt_ = true;
}

void PidObject::plantStateCallback(const nav_msgs::Odometry& state_msg)
{
  plant_state_ = state_msg;
...
void PidObject::doCalcs()
{
  ...
  if (new_state_or_setpt_)
  {
    ...
    error_.at(0) = setpoint_.pose.position.z- plant_state_.pose.pose.position.z;  // Current error goes to slot 0
    ...
    control_effort_.linear.z = proportional_ + integral_ + derivative_;

    // Apply saturation limits
    if (control_effort_.linear.z > upper_limit_)
      control_effort_.linear.z= upper_limit_;
    else if (control_effort_.linear.z < lower_limit_)
      control_effort_.linear.z = lower_limit_;

    // Publish the stabilizing control effort if the controller is enabled
    if (pid_enabled_)
    {
      control_msg_.data = control_effort_.linear.z;
